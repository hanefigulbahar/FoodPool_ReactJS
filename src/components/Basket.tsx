...
const Basket = () => {
    ...
    const total = basket.reduce((acc, item) => acc + item.amount * item.fee, 0) + deliveryFee
            ...
            {basket.length > 0 ?
                <>
                    ...
                    <div className='mx-auto w-80 h-1/4 border-dashed border-t-2 border-gray-400/5 '>
                        ...
                        <Link to={"order"} className='flex justify-center items-center m-auto my-3 h-14 w-80 bg-green-500/60 rounded-xl text-white'>Checkout</Link>
                    </div>
                </>
                : <div>Empty</div>
            }
