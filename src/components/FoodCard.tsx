...
import { Food, RestaurantByID } from '../types/products'

const FoodCard = (props: { data: RestaurantByID }) => {
    ...
    return (
        <>
            {props.data.restaurant?.menu?.map((data: Food) =>
                <div key={data.id} className="relative flex justify-center items-end m-auto text-white h-80 w-60" >
                    <img src={data.image} alt={data.image} className=' absolute top-0 h-36 w-36  rounded-full object-cover' />
                    <div className='flex justify-center pt-10 max-h-72 w-60 rounded-lg shadow-md text-black /60 border border-white hover:border-orange-200'>
                        <div className='flex flex-col justify-center text-center gap-2 w-72 mt-12 p-8'>
                            ...
                            <div className='flex justify-between text-2xl text-orange-300'>
                                ...
                                <button onClick={e => addBasket(data)}><BsFillPlusCircleFill /></button>
                            </div>
                        </div>
                    </div>
                </ div>
